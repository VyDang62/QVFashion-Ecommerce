...
const props = defineProps({
    flashSales: { type: Object, default: () => ({ data: [] }) },
...
const getCampaignProgress = (item) => {
    ...
    if (!item.is_active) return { label: 'Đã tắt', class: 'light' };
    if (now < start) return { label: 'Sắp diễn ra', class: 'primary' };
    if (now >= start && now <= end) return { label: 'Đang diễn ra', class: 'success' };
    return { label: 'Đã kết thúc', class: 'error' };
};

const handleRestore = (id) => {
    router.post(route('admin.flashsales.restore', id), {}, { preserveScroll: true });
};

const handleForceDelete = (id) => {
    router.delete(route('admin.flashsales.forcedelete', id), {}, { preserveScroll: true });
};

</script>

<template>
    <Head title="Quản lý Flash Sale" />
    <AdminLayout>
        ...
        <div class="space-y-6">
            <div class="flex items-center justify-between">
                ...
                <Link v-if="status === 'active' && can('flashsales.create')" :href="route('admin.flashsales.create')" 
            ...
            <DataTable 
                :title="status === 'trash' ? 'Flash Sale đã xóa' : 'Danh sách Flash Sale'"
                :headers="tableHeaders" 
                :items="flashSales?.data" 
                :pagination="flashSales" 
                v-model:search="searchTerm" 
                v-model:per-page="perPage"
                searchPlaceholder="Tìm tên Flash Sale..."
            >
                <template #row="{ item }">
                    <td class="px-5 py-4">
                        <p class="font-bold text-gray-800 text-md">{{ item.name }}</p>
                        <p class="text-[10px] text-gray-600 mt-0.5">ID: #FS-{{ item.id }}</p>
                    </td>
                    ...
                    <td class="px-5 py-4 text-sm font-medium text-gray-700">
                        {{ item.items_count || 0 }} sản phẩm
                    </td>
                    <td class="px-5 py-4">
                        <Badge variant="solid" size="sm" :color="getCampaignProgress(item).class">
                            {{ getCampaignProgress(item).label }}
                        </Badge>
                    </td>
                    ...
                    <td class="px-5 py-4 text-right">
                        <div class="flex items-center justify-begin gap-3">   
                            <template v-if="status === 'active'">
                                <Link v-if="can('flashsales.view')" :href="route('admin.flashsales.show', item.id)" class="text-gray-400 hover:text-blue-600">
                                ...
                                <Link v-if="can('flashsales.edit')" :href="route('admin.flashsales.edit', item.id)" class="text-gray-400 hover:text-blue-600 transition-colors">
                                ...
                                <DeleteAction
                                    v-if="can('flashsales.delete')"
                                    :message="`Bạn có chắc chắn muốn tạm xóa Flash Sale ${item.name}?`" 
                                    :item="item" 
                                    routeName="admin.flashsales.destroy" 
                            ...
                            <template v-else>
                                <ConfirmAction
                                    v-if="can('flashsales.delete')"
                                    title="Khôi phục Flash Sale"
                                    :message="`Khôi phục lại Flash Sale ${item.name}?`"
                                    variant="primary"
