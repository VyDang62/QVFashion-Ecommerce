...
watch([searchTerm, perPage, status], debounce(([newSearch, newPerPage, newStatus]) => {
    router.get(route('admin.producttypes.index'), { 
        search: newSearch, 
        perPage: newPerPage, 
        status: newStatus 
    }, {
...
const handleRestore = (id) => {
    router.post(route('admin.producttypes.restore', id), {}, {
        preserveScroll: true,
    });
};

const handleForceDelete = (id) => {
    router.delete(route('admin.producttypes.forcedelete', id), {}, {
        preserveScroll: true,
    });
};
</script>
    ...
    <AdminLayout>
        ...
        <div class="space-y-6">
            <div class="flex items-center justify-between">
                ...
                <Link v-if="status === 'active' && can('product-types.create')" 
            ...
            <DataTable 
                :title="status === 'active' ? 'Danh sách loại sản phẩm' : 'Loại sản phẩm đã xóa'"
                        ...
                        <div class="flex items-center justify-begin gap-3">
                            <template v-if="status === 'active'">
                                ...
                                <DeleteAction
                                    ...
                                    :message="`Bạn có chắc chắn muốn tạm xóa loại sản phẩm ${item.type_name}?`"
                                    :item="item"
                            ...
                            <template v-else>
                                <ConfirmAction
                                    v-if="can('product-types.delete')"
                                    title="Khôi phục"
                                    :message="`Bạn có muốn khôi phục lại loại sản phẩm ${item.type_name}?`"
                                    variant="primary"
                                ...
                                <ConfirmAction
                                    v-if="can('product-types.delete')"
                                    title="Xóa vĩnh viễn"
                                    :message="`Cảnh báo: Bạn có muốn xóa vĩnh viễn loại sản phẩm ${item.type_name}? Hành động này không thể hoàn tác!`"
                                    variant="danger"
